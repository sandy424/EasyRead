export default function Home() {
  return (
    <div className="bg-teal-50 w-full flex flex-col items-center p-12 font-ko">
      <h1 className="font-bold text-xl text-teal-900">글을 쉽게 읽을 수 있어요</h1>
      <p className="flex flex-col items-center mt-8 text-center">
        <span>이런 글을 쉽게 바꿔드립니다</span>
        <span>
          <span className="text-teal-600 font-bold">사진을 넣으면</span>, 글을 쉽게 바꿔드려요
        </span>
      </p>
      ...
      {/* 사진 업로드 영역 */}
      <h2 className="font-bold text-xl mt-10 text-teal-900">사진을 넣어주세요</h2>
      <div className="border border-transparent rounded-xl bg-white w-[800px] p-4 mt-8">
        <div className="border border-gray-300 hover:border-blue-400 border-dashed w-full rounded-xl flex flex-col items-center justify-center p-10">
          ...
          <input type="file" accept=".jpg,.png,.jpeg" id="file" className="mt-12 hidden" />
          <label
            htmlFor="file"
            className="px-5 py-3 bg-teal-100 text-gray-700 font-medium rounded-3xl cursor-pointer hover:bg-teal-200 mt-8 flex gap-2 items-center"
          >
      ...
      {/* 글 바꾸기 버튼 */}
      <button className="px-5 py-2 border border-transparent bg-teal-700 rounded-lg text-white font-regular text-base cursor-pointer hover:bg-teal-800 mt-8">글 바꾸기</button>
    </div>
  );
}
